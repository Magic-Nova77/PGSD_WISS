...
def make_model(opts, classes=None, sim=None):
    norm = get_norm(opts)
    ...
    if classes is not None:
        model = IncrementalSegmentationModule(body, head, head_channels, classes=classes, sim=sim)
    else:
...
class IncrementalSegmentationModule(nn.Module):
    ...
    def __init__(self, body, head, head_channels, classes, sim):
        ...
        # self.mask = torch.nn.Parameter(torch.rand([2048, 32, 32]), requires_grad=True)
